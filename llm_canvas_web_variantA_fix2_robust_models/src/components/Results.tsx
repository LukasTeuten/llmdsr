...
type SortKey =
  | "model_name" | "developer_name" | "country_of_origin"
  | "final_score" | "segment_label"
...
export function Results(props: {
  ...
  return (
    <div className="card cardPad">
      ...
      <p className="p">
        Höherer Score ist besser. Tokenkosten und Leistungswerte sind Proxy-Daten. Sortieren: Klick auf Spaltenkopf.
      </p>
        ...
        <table>
          <thead>
            <tr>
              ...
              <th onClick={() => onSort("final_score")}>Final Score</th>
              <th onClick={() => onSort("segment_label")}>Kosten-Segment</th>
              <th onClick={() => onSort("plan_match")}>Zahlungsmodell (Match)</th>
              <th onClick={() => onSort("input_token_price")}>Tokenkosten Input</th>
              <th onClick={() => onSort("output_token_price")}>Tokenkosten Output</th>
              {PERF_DIMS.map(d => (
                <th key={d}>{toDeDim(d)}</th>
              ))}
            </tr>
          </thead>
          <tbody>
