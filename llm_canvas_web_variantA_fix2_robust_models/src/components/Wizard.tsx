...
// Robustness: akzeptiere sowohl ein Array als auch { models: [...] }.
// Falls beides nicht passt, fällt die UI nicht komplett aus.
const modelList = Array.isArray((props as any).models)
  ? ((props as any).models as any[])
  : (Array.isArray((props as any)?.models?.models) ? (props as any).models.models : []);

const models = modelList as any[];
  ...
  const developers = useMemo(() => {
    const s = new Set(models.map(m => m.developer_name).filter(Boolean));
    return [...s].sort((a,b) => a.localeCompare(b,"de"));
  }, [models]);

  const countries = useMemo(() => {
    const s = new Set(models.map(m => m.country_of_origin).filter(Boolean));
    return [...s].sort((a,b) => a.localeCompare(b,"de"));
  }, [models]);
...
{models.length === 0 ? (
  <div style={{padding: "12px 14px", border: "1px solid #e5e7eb", borderRadius: 12, background: "#fff7ed", marginBottom: 12}}>
    <strong>Hinweis:</strong> Es konnten keine Modelldaten geladen werden. Prüfe bitte, ob <code>/models.json</code> im Deploy enthalten ist und ob das Format <code>[...]</code> oder <code>{'{'} "models": [...] {'}'}</code> ist.
  </div>
) : null}
